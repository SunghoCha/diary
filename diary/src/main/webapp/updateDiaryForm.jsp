...
<%
System.out.println("=============== updateDiaryForm ===============");
// 0. 로그인(인증) 분기
// diary.login.my_session => 'OFF' => redirect("loginForm.jsp")
	System.out.println("[updateDiaryForm] session-param loginMember : " + session.getAttribute("loginMember"));
...
/* 	String sql1 = "select my_session mySession from login";
	...
	System.out.println("[updateDairyForm] request-param diaryDate : " + diaryDate);

	String sql2 = "SELECT * FROM diary WHERE diary_date = ?;";
	PreparedStatement psmt2 = null;
	ResultSet rs2 = null;
	psmt2 = con.prepareStatement(sql2);
	psmt2.setString(1, diaryDate);
	System.out.println("[updateDairyForm] binging query : " + psmt2);
...
<body>
	...
	<form method="post" action="/diary/updateDiaryAction.jsp">
		...
		%>
		<div>
			title : <input class="login" type="text" name="title" value='<%=rs2.getString("title")%>'>
		</div>
		<div>
			date : <input class="login" value="<%=diaryDate%>" type="text" name="diaryDate" readonly="readonly">
		</div>
		<div>
			<% 
			...
			%>
						<select class="login custom-btn btn-8" name="weather" disabled="disabled">
							<option value="맑음" selected="selected">sunny</option>
			...
			%>
						<select class="login custom-btn btn-8" name="weather" disabled="disabled">
							...
							<option value="흐림" selected="selected">cloudy</option>
							<option value="비">rainy</option>
							<option value="눈">snowy</option>
						</select>
			<% 		
			...
			%>
						<select class="login custom-btn btn-8" name="weather" disabled="disabled">
							<option value="맑음">sunny</option>
							<option value="흐림">cloudy</option>
							<option value="비" selected="selected">rainy</option>
							<option value="눈">snowy</option>
						</select>
			<% 		
					} else {
			%>
						<select class="login custom-btn btn-8" name="weather" disabled="disabled">
							...
							<option value="흐림">cloudy</option>
							<option value="비">rainy</option>
							<option value="눈" selected="selected">snowy</option>
						</select>
		...
		<div>
			<textarea class="login" rows="7" cols="50" name="content"><%=rs2.getString("content")%></textarea>
		</div>
		<div>
